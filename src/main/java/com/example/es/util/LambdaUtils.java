...
public final class LambdaUtils {
    ...
    /**
     * 解析 lambda 表达式, 该方法只是调用了 {@link SerializedLambda#resolve(com.example.es.function.EsFunction)} 中的方法，在此基础上加了缓存。
     * 该缓存可能会在任意不定的时间被清除
     *
     * @param func 需要解析的 lambda 对象
     * @param <T>  类型，被调用的 Function 对象的目标类型
     * @return 返回解析后的结果
     * @see SerializedLambda#resolve(com.example.es.function.EsFunction)
     */
    public static <T> SerializedLambda resolve(EsFunction<T, ?> func) {
     ...
     */
    public static void installCache(IndexInfo indexInfo) {
        COLUMN_CACHE_MAP.put(indexInfo.getEntityType().getName(), createColumnCacheMap(indexInfo));
    }
    ...
    public static Map<String, ColumnCache> getColumnMap(Class<?> clazz) {
        return COLUMN_CACHE_MAP.computeIfAbsent(clazz.getName(), key -> {
            IndexInfo info = IndexHelper.getIndexInfo(clazz);
